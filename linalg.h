...
public:
	...
	inline vec3T(const vec3T<T> &r)  {memcpy(x,r.x,3*sizeof(T));}
	...
	inline T			norm()	{return sqrt(x[0]*x[0]+x[1]*x[1]+x[2]*x[2]);}		// норма текущего вектра 
	inline T			sqnr()	{return (x[0]*x[0]+x[1]*x[1]+x[2]*x[2]);}			// квадрат норма текущего вектра 
	inline T			scpr(const vec3T<T> &a)	{return (x[0]*a.x[0]+x[1]*a.x[1]+x[2]*a.x[2]);}		// скалярное произведение текущего вектра и вектора 'a'
	inline vec3T<T>&	crss(const vec3T<T> &a,vec3T<T>	&b)	// векторное произведение [a x b] результат в текущий вектор
		{x[0]	= a.x[1]*b.x[2]-a.x[2]*b.x[1];x[1]	= a.x[2]*b.x[0]-a.x[0]*b.x[2];x[2]	= a.x[0]*b.x[1]-a.x[1]*b.x[0];return *this;	}
...
public:
	...
	inline vec2T(const vec2T &r)  {memcpy(x,r.x,2*sizeof(T));}

	//static const int T_size	= 2*sizeof(T);
	
	inline T	scpr(const vec2T<T> &a)	{return (x[0]*a.x[0]+x[1]*a.x[1]);}				// скалярное произведение текущего вектора и вектора 'a'
	inline T	crss(const vec2T<T> &a)	{return x[0]*a.x[1]-x[1]*a.x[0];}	// векторное произведение текущего вектора и вектора 'a'
...
//	void pad(T &y);		// добавить другой массив в конец данного массива в случае если элемент массива имеет поле указателя
	...
	vecT<T> &	operator = (const vecT<T> &a) {if(N<a.N) setmem(a.N); N=a.N; for(int i = 0;i<N;++i) x[i]=a.x[i]; return *this;}//{pst(a);return *this;}
